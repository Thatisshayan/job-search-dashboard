...
export async function importVerifiedListings(
  userId: number,
...
export async function importVerifiedListingBatch(
  userId: number,
  listings: VerifiedListing[]
) {
  if (!listings.length)
    throw new Error("At least one verified listing is required");
  ...
  if (!profile[0] || !settings[0])
    throw new Error("Candidate profile and search settings are required");

  const runInsert = await db
    .insert(jobRuns)
    .values({ userId, status: "running", sourcesChecked: 1 });
  ...
  try {
      ...
      await db
        ...
        .where(
          and(
            ...
            eq(sourceConfigs.name, listings[0].sourceName)
          )
        )
        .limit(1)
    )[0];
    if (!source)
      throw new Error(
        `Source configuration not found: ${listings[0].sourceName}`
      );
    if (!source.enabled)
      throw new Error(`Source is disabled: ${listings[0].sourceName}`);
    ...
    if (shortlisted.length) {
      await db.insert(shortlistEntries).values(
    ...
    await db
      ...
      .set({
        lastStatus: "Verified fallback imported",
