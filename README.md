...
| `NVIDIA_API_KEY` / `NVIDIA_BASE_URL`                | LLM provider used by `server/_core/llm.ts` (`invokeLLM`/`listLLMModels`), an OpenAI-compatible client against [Nvidia's own API](https://build.nvidia.com) (`https://integrate.api.nvidia.com/v1` by default). Not required to boot today; needed for the Telegram-agent work described in `docs/telegram-agent/`. Previously routed through OpenRouter — see `docs/telegram-agent/DECISIONS.md` D4's 2026-09-13 update for why that was dropped. |
| `ADZUNA_APP_ID` / `ADZUNA_APP_KEY` / `ADZUNA_DEFAULT_COUNTRY` | Job-search API used by `server/jobSearch/adzuna.ts`. Free, no card required ([sign up](https://developer.adzuna.com)). Optional — without it, the Telegram bot still onboards users but skips the automatic search. |
