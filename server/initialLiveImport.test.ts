...
import {
  importVerifiedListingBatch,
  type VerifiedListing,
} from "./verifiedListingImport";

const liveImport = process.env.LIVE_JOBBANK_IMPORT === "1" ? it : it.skip;
const sourcePath = fileURLToPath(
  new URL(
    "../docs/research/initial_verified_jobbank_listings.json",
    import.meta.url
  )
);
const listings = JSON.parse(readFileSync(sourcePath, "utf8")) as Array<
  Omit<VerifiedListing, "postedAt" | "expiresAt"> & {
    postedAt: string;
    expiresAt?: string;
  }
>;
...
describe("initial verified Job Bank import", () => {
  liveImport(
    "publishes the independently reviewed live records without any employer submission",
    async () => {
      ...
      const result = await importVerifiedListingBatch(
        owner.id,
