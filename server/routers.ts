...
const ownerProcedure = protectedProcedure.use(({ ctx, next }) => {
  if (!isWorkspaceOwner(ctx.user, ENV.ownerOpenId)) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "This private dashboard is restricted to its owner.",
    });
  }
  return next({ ctx });
});
...
export const appRouter = router({
  ...
  dashboard: router({
    accessStatus: protectedProcedure.query(({ ctx }) => ({
      isOwner: isWorkspaceOwner(ctx.user, ENV.ownerOpenId),
    })),
    overview: publicProcedure.query(async () =>
      getDashboardOverview(await ensurePublicWorkspaceInitialized())
    ),
    shortlist: publicProcedure
      ...
      .query(async ({ input }) =>
        listShortlist(
          await ensurePublicWorkspaceInitialized(),
          input.dateKey ?? getLocalDateKey("America/Toronto")
        )
      ),
    history: publicProcedure.query(async () =>
      listJobHistory(await ensurePublicWorkspaceInitialized())
    ),
    runs: publicProcedure.query(async () =>
      listRuns(await ensurePublicWorkspaceInitialized())
    ),
    profile: publicProcedure.query(async () =>
      getProfile(await ensurePublicWorkspaceInitialized())
    ),
    settings: publicProcedure.query(async () =>
      getSettings(await ensurePublicWorkspaceInitialized())
    ),
    updateSettings: ownerProcedure
      .input(settingInput)
      .mutation(({ ctx, input }) => updateSettings(ctx.user.id, input)),
    ...
    setSourceEnabled: ownerProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) =>
        updateSourceEnabled(ctx.user.id, input.sourceId, input.enabled)
      ),
    addSource: ownerProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) => addSource(ctx.user.id, input)),
    ...
    prepareApplication: ownerProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const application = await prepareApplicationForTelegram(
          ctx.user.id,
          input.jobId
        );
        return { application };
    ...
    importVerifiedListings: ownerProcedure
      .input(
        z.object({ listings: z.array(verifiedListingInput).min(1).max(20) })
      )
      .mutation(({ ctx, input }) =>
        importVerifiedListingBatch(ctx.user.id, input.listings)
      ),
    previewScore: ownerProcedure
      .input(
        z.object({
          ...
          locationKm: z.number().int().optional(),
          originalApplyUrl: z.string().url().optional(),
          postedAt: z.date().optional(),
          status: z
            .enum(["active", "expired", "unavailable", "stale"])
            .optional(),
          skillMatches: z.array(z.string()).optional(),
          seniorityMatch: z.enum(["strong", "partial", "weak"]).optional(),
